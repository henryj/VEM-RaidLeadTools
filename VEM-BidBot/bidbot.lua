...
VEM_BidBot_ItemHistory = {}		-- ItemHistory

VEM_BidBot_Settings = {}
local settings = default_settings

local L = VEM_BidBot_Translations

local revision = ("$Revision: 112 $"):sub(12, -3)
...
do 
	...
	local function creategui()
		local panel = VEM_RaidLeadPanel:CreateNewPanel(L.TabCategory_BidBot, "option")
		do
			...
			checkclients:SetScript("OnClick", function(self) 
				if VEM:IsInRaid() and IsInGroup(LE_PARTY_CATEGORY_HOME) then
					SendAddonMessage("VEM_BidBot", "showversion!", "RAID")
				else
					VEM:AddMsg(L.Local_NoRaid)
				end
			end)
			...
			resetbidbot:SetScript("OnClick", function(self) 
				table.wipe(VEM_BidBot_ItemHistory)
				table.wipe(VEM_BidBot_Settings)
				addDefaultOptions(settings, default_settings)

				VEM_GUI_OptionsFrame:Hide()
				VEM_GUI_OptionsFrame:Show()				
			end)
			...
			enabled:SetScript("OnClick", function(self) 
				if VEM:IsInRaid() and IsInGroup(LE_PARTY_CATEGORY_HOME) then
					if settings.enabled then
						SendAddonMessage("VEM_BidBot", "bye!", "RAID")
					else
						SendAddonMessage("VEM_BidBot", "Hi!", "RAID")
					end
				end
		...
		do	
			...
			history:SetScript("OnShow", function(self)
				if #VEM_BidBot_ItemHistory > 0 then
					self:SetMaxLines((#VEM_BidBot_ItemHistory*4)+1)
					for k,itembid in pairs(VEM_BidBot_ItemHistory) do
	...
	VEM:RegisterOnGuiLoadCallback(creategui, 11)
end
...
function sendchatmsg(msg)
	...
	elseif settings.chatchannel == "GUILD" or settings.chatchannel == "RAID" or settings.chatchannel == "PARTY" or settings.chatchannel == "OFFICER" then
		...
		if chanid > 4 then
			...
			VEM:AddMsg(L.Error_ChanNotFound:format(msg))
		end
	end
end
...
function AddBid(bidder, bid)
	...
	if settings.bidtyp_open then
		...
		SendChatMessage("<VEM> "..L.Prefix..L.Whisper_Bid_OK:format(bid), "WHISPER", nil, bidder)
	end
end
...
function DelBid(bidder)
	...
	SendChatMessage("<VEM> "..L.Prefix..L.Whisper_Bid_DEL, "WHISPER", nil, bidder)
end
...
function AuctionEnd()
	...
	table.insert(VEM_BidBot_ItemHistory, Itembid)
	...
	-- Sync History
	SendAddonMessage("VEM_BidBot", "ITEM:"..select(2, strsplit(":", Itembid.item))..":"..Itembid.points..":("..msg..")", "RAID")
	--SendAddonMessage("VEM_BidBot", "ITEM:"..select(2, strsplit(":", Itembid.item))..":"..Itembid.points..":("..msg..")", "GUILD")
	...
	if #BidBot_Queue then
		-- Shedule next Item
		sendchatmsg("--- --- --- --- ---")
		VEM:Schedule(1.5, StartBidding)
	end
end	

function StartBidding()
	if BidBot_InProgress then
		VEM:AddMsg(L.Prefix..L.Whisper_InUse:format(CurrentItem))
	else
		...
		VEM:Schedule((settings.duration / 6) * 5, function() 
			sendchatmsg(L.Prefix..L.Message_DoBidding:format(ItemLink, math.floor(settings.duration / 6)))
			end)
		VEM:Schedule(settings.duration / 2, function() 
			sendchatmsg(L.Prefix..L.Message_DoBidding:format(ItemLink, math.floor(settings.duration / 2)))
			end)
		VEM:Schedule(settings.duration, AuctionEnd)
	end
end

do
	local hiddenedit = CreateFrame('EditBox', "VEM_DKP_PopupExtension", UIParent)
	...
	-- /script StaticPopup_Show("VEM_DKP_ACCEPT", "item")	
	StaticPopupDialogs["VEM_DKP_ACCEPT"] = {
		...
		OnAccept = function(self)
			...
			if hiddenedit.itemtable.points and hiddenedit.itemtable.points > 0 then
				...
				VEM_AddItemToDKP(hiddenedit.itemtable)
			end
	...
	function DoInjectToDKPSystem(itemtable)
		if VEM_DKP_System_Settings and VEM_DKP_System_Settings.enabled then
			...
			StaticPopup_Show("VEM_DKP_ACCEPT", itemtable.item)
		end
	end
end
...
do 
	...
	local function amIactive()
		if not VEM:IsInRaid() and IsInGroup(LE_PARTY_CATEGORY_HOME) then return false end
	
		local myname = UnitName("player")
	
		for k,v in pairs(bidbot_clients) do
			if VEM:GetRaidRank(k) >= 2 then	-- raidleader gefunden
				if k == myname then
		...
		for k,v in pairs(bidbot_clients) do
			if UnitIsConnected(VEM:GetRaidUnitId(k)) and k < myname then
	...
	local function OnMsgRecived(msg, name, nocheck)
		if settings.enabled and VEM:IsInRaid() and msg and string.find(string.lower(msg), "^!bid ") then
			if not nocheck and not amIactive() then
				return false
			end
			if name ~= UnitName("player") and VEM:GetRaidUnitId(name) == "none" then
			...
			if string.find(ItemLink, "|c(%x+)|Hitem:(.-)|h%[(.-)%]|h|r") then
			   ...
			   if BidBot_InProgress then
				SendChatMessage("<VEM>"..L.Prefix..L.Whisper_Queue, "WHISPER", nil, name)
			   else
				VEM:Schedule(1.5, StartBidding)
			   end
			end
	...
	local BidBot_Frame = CreateFrame("Frame", "VEM_BidBotFrame", UIParent)
	...
	BidBot_Frame:SetScript("OnEvent", function(self, event, ...)
		local arg1, arg2 = ...
		if event == "ADDON_LOADED" and select(1, ...) == "VEM-RaidLeadTools" then
			-- Update settings of this Addon
			settings = VEM_BidBot_Settings
			addDefaultOptions(settings, default_settings)

			VEM:RegisterCallback("raidJoin", function(event, name)
				if settings.enabled and name and name == UnitName("player") then 
					SendAddonMessage("VEM_BidBot", "Hi!", "WHISPER", name)
				end 
			end)
			...
			if VEM_BidBot_Translations[GetLocale()] then 
				L = VEM_BidBot_Translations[GetLocale()]
			end
		...
		elseif settings.enabled and event == "CHAT_MSG_ADDON" then
			...
			if prefix == "VEM_BidBot" then
				if msg == "Hi!" then
					bidbot_clients[sender] = true
					if settings.enabled and channel == "RAID" then
						SendAddonMessage("VEM_BidBot", "Hi!", "WHISPER", sender)				
					end
				...
				elseif msg == "showversion!" then
					if channel == "RAID" then
						SendAddonMessage("VEM_BidBot", "version: r"..tostring(revision), "WHISPER", sender)
					end

				elseif msg:sub(0, 9) == "version: " then
					VEM:AddMsg( L.Local_Version:format(sender, msg:sub(9)) )

				elseif msg:sub(0, 5) == "ITEM:" and sender ~= UnitName("player") then
					if VEM:GetRaidUnitId(sender) ~= "none" and not channel == "RAID" then return end
					if VEM:GetRaidUnitId(sender) == "none" and not channel == "GUILD" then return end
