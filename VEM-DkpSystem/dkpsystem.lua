...
VEM_DKP_System_Settings = {}
local settings = default_settings

local L = VEM_DKP_System_Translations
...
do
	local function creategui()
		local panel = VEM_RaidLeadPanel:CreateNewPanel(L.TabCategory_DKPSystem, "option")
		do
			...
			button:SetScript("OnClick", function(self)
				if settings.working_in > 0 then
					...
					if GetNumGroupMembers() == 0 then
						VEM:AddMsg(L.Local_NoRaidPresent)
					else
						RaidStart()
					end
				end
			...
			button:SetScript("OnClick", function(self)
				if neweventpoints:GetNumber() <= 0 or neweventdescr:GetText() == "" then 
					VEM:AddMsg(L.Local_NoInformation)
				else
					local event = {
						event_type = "custom",
					...
					CreateEvent(event)
					VEM:AddMsg(L.Local_EventCreated)
		...
		do
					...
					for i=1, #settings.history, 1 do
						...
						if #raid.events > 0 then
							for k,event in pairs(raid.events) do
								...
								local link = "|HVEM:showdkp:"..i..":"..k.."|h|cff3588ff[show]|r|h"
			...
			history:SetScript("OnHyperlinkClick", function(self, link, string, button, ...)
				...
				if linkType == "VEM" and arg1 == "showdkp" and history and event then
					history = tonumber(history)
					event = tonumber(event)
					--VEM:AddMsg("Opening DKP String for Event: "..event.." from Raid "..history)
...
do
	...
	StaticPopupDialogs["VEM_EXPORT_DKP_STRING"] = {
		text = "DKP String - %s",
		button1 = ACCEPT,
		button2 = CANCEL,
		hasEditBox = 1,
	...
	function ShowExportString(raid_id, event_id) 
		...
		StaticPopup_Show("VEM_EXPORT_DKP_STRING", "raid")
	end
end
...
function GetRaidList()
	...
	if settings.sb_as_raid then
		for k,v in pairs(VEM_Standby_Settings.sb_users) do
...
function VEM_AddItemToDKP(itemtable)
	if not itemtable or type(itemtable) ~= "table" then
		VEM:AddMsg("Function VEM_AddItemToDKP(itemtable) call failed. Debugstack: "..debugstack())
		return false

	elseif settings.working_in == 0 then
		VEM:AddMsg(L.LocalError_AddItemNoRaid)
		return false
	end
...
function CreateEvent(event)
	...
	if #event.members then
		...
		VEM:AddMsg(L.Local_Debug_NoRaid)
	end
end

function VEM_DKP_BossKill(event, bossmod)
...
VEM:RegisterCallback("kill", VEM_DKP_BossKill)
...
do
	local mainframe = CreateFrame("frame", "VEM_DKP_System", UIParent)
	mainframe:SetScript("OnEvent", function(self, event, ...)
		if event == "ADDON_LOADED" and select(1, ...) == "VEM-RaidLeadTools" then
			...
			settings = VEM_DKP_System_Settings
			addDefaultOptions(settings, default_settings)
		end
	end)
	mainframe:SetScript("OnUpdate", function(self, e)
		...
		if time() - settings.lastevent > (settings.time_to_count*60) then
			VEM:AddMsg(L.Local_TimeReached)
			CreateEvent({
				event_type = "",
				zone = GetRealZoneText(),
