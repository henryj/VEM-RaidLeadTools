...
local mainframe = CreateFrame("Frame", "VEM_Raidleadtool", UIParent)

local function createpanel()
	if GetLocale() ~= "zhTW" then
		VEM_RaidLeadPanel = VEM_GUI:CreateNewPanel("Raidlead Tools - r"..Revision, "option")
	else
		VEM_RaidLeadPanel = VEM_GUI:CreateNewPanel("團隊隊長工具 - r"..Revision, "option")
	end

	local area = VEM_RaidLeadPanel:CreateArea(L.Area_Raidleadtool, nil, 180, true)
...
do 
	...
	mainframe:SetScript("OnEvent", function(self, event, ...)
		if event == "ADDON_LOADED" and select(1, ...) == "VEM-RaidLeadTools" then
			...
			settings = VEM_RaidLead_Settings
			addDefaultOptions(settings, default_settings)

			-- StickyIcons
			VEM:RegisterCallback("pull", scanIcons)
			VEM:RegisterCallback("wipe", combat_end)
			VEM:RegisterCallback("kill", combat_end)

			-- WarnforLootmaster
			VEM:RegisterCallback("pull", function()
				if VEM_BidBot_Translations.WarnWhenNoLootmaster and GetLootMethod() ~= "master" then
					VEM:AddMsg(L.Warning_NoLootMaster)
				end
			end)			
		end
	end)
...
VEM:RegisterOnGuiLoadCallback(createpanel, 10)
