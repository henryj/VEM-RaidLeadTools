...
local mainframe = CreateFrame("frame", "VEM_StandyByBot", UIParent)
...
VEM_Standby_Settings = {}
local settings = default_settings

local L = VEM_StandbyBot_Translations
local sbbot_clients = {}
...
do 
	local function creategui()
		local panel = VEM_RaidLeadPanel:CreateNewPanel(L.TabCategory_Standby, "option")
		do
			...
			enabled:SetScript("OnClick", function(self) 
				if VEM:IsInRaid() then
					if settings.enabled then
						SendAddonMessage("VEM_SbBot", "bye!", "RAID")
					else
						SendAddonMessage("VEM_SbBot", "Hi!", "RAID")
					end
				end
			...
			checkclients:SetScript("OnClick", function(self) 
				if VEM:IsInRaid() then
					SendAddonMessage("VEM_SbBot", "showversion!", "RAID")
				else
	...
	VEM:RegisterOnGuiLoadCallback(creategui, 13)
end
...
local function setStandby(name, nowsb)
	...
	if nowsb then
		...
		if amIactive() then 
			SendAddonMessage("VEM_SbBot", "!sb add "..name, "RAID")
		end
		return true
		...
		if amIactive() then
			SendAddonMessage("VEM_SbBot", "!sb del "..name, "RAID")
		end
...
function amIactive()
	if not VEM:IsInRaid() then return false end
...
local function AddStandbyMember(name, quiet)
	...
	if settings.sb_users[name] == nil then
		-- a brand new member
		if not quiet then
			VEM:AddMsg( L.Local_AddedPlayer:format(name) )
			SendChatMessage("<VEM> "..L.AddedSBUser, "WHISPER", nil, name)
		end
		setStandby(name, true)
	else
		-- member allready on the list
		if not quiet then 
			SendChatMessage("<VEM> "..L.UserIsAllreadySB, "WHISPER", nil, name)
		end
	end
end
...
local function RemoveStandbyMember(name, quiet)
	...
	if settings.sb_users[name] == nil then
		...
		if not quiet then
			VEM:AddMsg( L.Local_RemovedPlayer:format(name) )

			local minutes, hours = raidtime(sbtime)
			SendChatMessage("<VEM> "..L.NoLongerStandby:format(hours or 0, minutes or 0), "WHISPER", nil, name)
		end
		return true
	end
end
...
do
	function SaveTimeHistory()
		...
		if not table_empty(settings.sb_times) then
			...
			VEM:AddMsg( L.SB_History_Saved:format(#settings.history) )
		else
			VEM:AddMsg( L.SB_History_NotSaved )
		end
	end

	VEM:RegisterCallback("raidLeave", function(event, name)
		if settings.enabled and name and select(2, IsInInstance()) ~= "pvp" and select(2, IsInInstance()) ~= "arena" then
			...
			elseif amIactive() and settings.send_whisper then
				SendChatMessage("<VEM> "..L.LeftRaidGroup, "WHISPER", nil, name)
			end
		end
	end)
	VEM:RegisterCallback("raidJoin", function(event, ...) return RemoveStandbyMember(...) end)
end
...
do	
	...
	mainframe:SetScript("OnEvent", function(self, event, ...)
		...
		elseif settings.enabled and event == "CHAT_MSG_WHISPER" and VEM:IsInRaid() then
			local msg, author = select(1, ...)
			if msg == "!sb" then
				if VEM:GetRaidUnitId(author) == "none" then
					AddStandbyMember( author )
				else
					SendChatMessage("<VEM> "..L.InRaidGroup, "WHISPER", nil, author)
				end
		...
		elseif settings.enabled and event == "CHAT_MSG_ADDON" then
			...
			if prefix ~= "VEM_SbBot" then return end
			if sender == UnitName("player") then return end

			if msg == "Hi!" then
				sbbot_clients[sender] = true
				if channel == "RAID" then
					SendAddonMessage("VEM_SbBot", "Hi!", "WHISPER", sender)				
				end

			elseif msg == "showversion!" then
				if channel == "RAID" then
					SendAddonMessage("VEM_SbBot", "version: r"..tostring(revision), "WHISPER", sender)
				end
			...
			elseif msg == "refresh!" and sbbot_clients[sender] and amIactive() then
				UpdateTimes()
				SendAddonMessage("VEM_SbBot", "cleanup!", "WHISPER", sender)

				for k,v in pairs(settings.sb_times) do
					SendAddonMessage("VEM_SbBot", "TIMES:"..v..":"..k, "WHISPER", sender)
				end
				for k,v in pairs(settings.sb_users) do
					SendAddonMessage("VEM_SbBot", "SBUSER:"..v..":"..k, "WHISPER", sender)
				end

			elseif msg == "cleanup!" then
				VEM:AddMsg( L.Local_CleanList:format(sender) )
				table.wipe(settings.sb_times)
				table.wipe(settings.sb_users)
		...
		if settings.enabled and event:sub(0, 9) == "CHAT_MSG_" and event ~= "CHAT_MSG_WHISPER" and event ~= "CHAT_MSG_ADDON" then
			...
			if active and msg == "!sb" then
				...
				SendChatMessage("<VEM> "..L.PostStandybyList.." "..output, "WHISPER", nil, author)
