...
local L = VEM_AutoInvite_Translations
...
VEM_AutoInvite_Settings = {}
local settings = default_settings


local settings = VEM_AutoInvite_Options
local mainframe = CreateFrame("Frame", "VEM_AutoInvite", UIParent)
...
do 
	...
	local function creategui()
		local panel = VEM_RaidLeadPanel:CreateNewPanel(L.TabCategory_AutoInvite, "option")
		do
...
--			keyword:SetScript("OnShow", 		function(self) self:SetText(settings.keyword) end)
		...
		do
			...
			resettool:SetScript("OnClick", function(self) 
				table.wipe(VEM_AutoInvite_Settings)
	...
	VEM:RegisterOnGuiLoadCallback(creategui, 15)
end
...
do
	...
	VEM:RegisterCallback("raidJoin", function(event, name)
		if settings.enabled and name and VEM:GetRaidRank() > 0 then
	...
	function DoInvite(name)
		...
		if not IsInRaid() then
			...
			if pplcount > 0 and not UnitIsGroupLeader("player") then
				VEM:AddMsg( L.InviteFailed:format(name) )
			else
				if pplcount < 4 then
					InviteUnit(name)

				elseif pplcount == 4 then
					VEM:AddMsg( L.ConvertRaid )
					ConvertToRaid()
					InviteUnit(name)
				end
			end

		elseif VEM:GetRaidRank() > 0 and pplcount < 40 then 
			InviteUnit(name)

		elseif VEM:GetRaidRank() == 0 then
			SendChatMessage("<VEM> "..L.WhisperMsg_NotLeader, "WHISPER", nil, name)

		elseif pplcount >=40 then
			SendChatMessage("<VEM> "..L.WhisperMsg_RaidIsFull, "WHISPER", nil, name)
		end
	end
	...
	mainframe:SetScript("OnEvent", function(self, event, ...)
		if event == "ADDON_LOADED" and select(1, ...) == "VEM-RaidLeadTools" then
			...
			settings = VEM_AutoInvite_Settings
			addDefaultOptions(settings, default_settings)
...
SLASH_VEMAUTOINVITE1 = "/inviteguild"
SlashCmdList["VEMAUTOINVITE"] = slashfunction 
